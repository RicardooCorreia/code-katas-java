package advent.of.code.y2025;

import org.junit.jupiter.api.Test;

import static org.assertj.core.api.Assertions.assertThat;

class Day2GiftShopPart2Test {

    private final Day2GiftShop subject = new Day2GiftShop();

    @Test
    void findInvalidIds_baseExample() {

        // Given
        String input = "11-22,95-115,998-1012,1188511880-1188511890,222220-222224," +
                "1698522-1698528,446443-446449,38593856-38593862,565653-565659," +
                "824824821-824824827,2121212118-2121212124";

        // When
        long result = subject.findInvalidIds(input);

        // Then
        assertThat(result)
                .isEqualTo(4174379265L);
    }

    @Test
    void findInvalidIds() {

        // Given
        String input = "851786270-851907437,27-47,577-1044,1184-1872,28214317-28368250,47766-78575,17432-28112,2341-4099,28969-45843,5800356-5971672,6461919174-6461988558,653055-686893,76-117,2626223278-2626301305,54503501-54572133,990997-1015607,710615-802603,829001-953096,529504-621892,8645-12202,3273269-3402555,446265-471330,232-392,179532-201093,233310-439308,95134183-95359858,3232278502-3232401602,25116215-25199250,5489-8293,96654-135484,2-17";

        // When
        long result = subject.findInvalidIds(input);

        // Then
        assertThat(result)
                .isEqualTo(22617871034L);
    }
}
