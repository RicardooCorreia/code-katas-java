...
public class Day7Laboratories {
    ...
    public int splitBeam(String input) {
        ...
        for (int lineIndex = 1; lineIndex < matrix.length; lineIndex++) {
            for (int columnIndex = 0; columnIndex < matrix[lineIndex].length; columnIndex++) {
                ...
                char currentSpot = matrix[lineIndex][columnIndex];
                char previousSpot = matrix[lineIndex - 1][columnIndex];
                if (previousSpot == '|' || previousSpot == 'S') {
                    if (currentSpot == '^') {
                        split += 1;
                        int leftPosition = columnIndex - 1;
                        int rightPosition = columnIndex + 1;
                        if (leftPosition >= 0) {
                            matrix[lineIndex][leftPosition] = '|';
                        }
