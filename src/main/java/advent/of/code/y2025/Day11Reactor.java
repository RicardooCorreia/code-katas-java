...
public class Day11Reactor {
    ...
    public int numOfPaths(String input) {
        ...
        return countPaths(paths, paths.get("you"));
    }

    private int countPaths(Map<String, List<String>> paths, List<String> nextConnections) {

        int currentCount = 0;
        for (String nextConnection : nextConnections) {
            if (Objects.equals(nextConnection, "out")) {
                currentCount++;
            } else {
                currentCount += countPaths(paths, paths.get(nextConnection));
            }
        }
