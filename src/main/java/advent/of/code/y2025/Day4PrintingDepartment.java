        ...
        for (int lineIndex = 0; lineIndex < lines.length; lineIndex++) {
            ...
            for (int columnIndex = 0; columnIndex < line.length(); columnIndex++) {
                ...
                if (line.charAt(columnIndex) == '@') {
                    ...
                    boolean hasSpaceBottom = bottomLineIndex < lines.length;

                    String topLine = hasSpaceTop ? lines[topLineIndex] : "";
                    String bottomLine = hasSpaceBottom ? lines[bottomLineIndex] : "";
                    if (hasSpaceLeft) {
                        if (line.charAt(leftColumnIndex) == '@') {
                            rollCount++;
                        }
                        if (hasSpaceTop && topLine.charAt(columnIndex - 1) == '@') {
                            rollCount++;
                        }
                        if (hasSpaceBottom && bottomLine.charAt(columnIndex - 1) == '@') {
                            rollCount++;
                        }
                    }
                    if (hasSpaceRight) {
                        if (line.charAt(rightColumnIndex) == '@') {
                            rollCount++;
                        }
                        if  (hasSpaceTop && topLine.charAt(columnIndex + 1) == '@') {
                            rollCount++;
                    ...
                    if (hasSpaceTop && topLine.charAt(columnIndex) == '@') {
                        rollCount++;
                    }
                    if (hasSpaceBottom && bottomLine.charAt(columnIndex) == '@') {
                        rollCount++;
                    }
