 ...
 */
public class Day11ReactorPart2 {

    Map<CacheKey, Long> cache = new HashMap<>();

    public long numOfPaths(String input) {
        Map<String, List<String>> paths = Arrays.stream(input.split("\n"))
                .map(s -> s.split(": "))
                .collect(Collectors.toMap(
                        strings -> strings[0],
                        strings -> Arrays.stream(strings[1].split(" ")).toList()));

        Set<String> visited = new HashSet<>();
        visited.add("svr");
        return calculatePaths(paths, "svr", visited, false, false);
    }

    private long calculatePaths(Map<String, List<String>> paths,
                                String currentNode,
                                Set<String> visited,
                                boolean hasSeenDac,
                                boolean hasSeenFft) {

        List<String> nextConnections = paths.get(currentNode);
        ...
        for (String nextConnection : nextConnections) {
            if (nextConnection.equals("out")) {
                if (hasSeenFft && hasSeenDac) {
                    count++;
                }
            } else if (!visited.contains(nextConnection)) {

                boolean newHasSeenDac = hasSeenDac || nextConnection.equals("dac");
                boolean newHasSeenFft = hasSeenFft || nextConnection.equals("fft");
                CacheKey cacheKey = new CacheKey(nextConnection, newHasSeenDac, newHasSeenFft);
                if (cache.containsKey(cacheKey)) {
                    count += cache.get(cacheKey);
                } else {

                    visited.add(nextConnection);

                    long possiblePaths = calculatePaths(paths, nextConnection, visited, newHasSeenDac, newHasSeenFft);
                    cache.put(cacheKey, possiblePaths);
                    count += possiblePaths;
                    visited.remove(nextConnection);
                }
            }
        }
    ...
    public record CacheKey(String node, boolean hasSeenDac, boolean hasSeenFft) {
