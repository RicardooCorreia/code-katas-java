...
Constraints:
    ...
    1 <= nums[i] <= 106
 */
public class NumberDistinctIntegersAfterReverseOperations {

    static class Solution {
        public int countDistinctIntegers(int[] nums) {

            return Math.toIntExact(Arrays.stream(nums)
                    .flatMap(value -> IntStream.of(value, inverseNumber(value)))
                    .distinct()
                    .count());
        }

        private static int inverseNumber(Integer value) {
