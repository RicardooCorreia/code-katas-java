    ...
    static class Solution {
        public String replaceWords(List<String> dictionary, String sentence) {
            ...
            for (String word : words) {
                result
                        .append(getDerivativeOrSame(word, dictionary))
                        .append(" ");
            }
            return result.toString().trim();
        }

        private String getDerivativeOrSame(String word, List<String> dictionary) {

            String result = word;
            for (String derivative : dictionary) {
                if (word.startsWith(derivative) && derivative.length() < result.length()) {
                    result = derivative;
                }
            }
            return result;
