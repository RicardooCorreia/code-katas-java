...
public class SubdomainVisitCount {
    ...
    static class Solution {
        public List<String> subdomainVisits(String[] domains) {
            ...
            Map<String, Long> result = getVisitCount(domains);
        ...
        private static Map<String, Long> getVisitCount(String[] domains) {
            Map<String, Long> result = new HashMap<>();
            for (String domain : domains) {
                String[] parts = domain.split(" ");
                String clickCount = parts[0];
                String fullDomain = parts[1];

                List<String> subDomains = getSubDomains(fullDomain);

                subDomains.forEach(subDomain -> {
                    result.putIfAbsent(subDomain, 0L);
                    result.put(subDomain, result.get(subDomain) + Long.parseLong(clickCount));
                });
            }
            return result;
        }

        private static List<String> getSubDomains(String fullDomain) {
